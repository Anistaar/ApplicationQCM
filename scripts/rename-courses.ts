#!/usr/bin/env ts-node
/**
 * scripts/rename-courses.ts
 * Analyse les fichiers src/cours/**/*.txt et propose de nouveaux noms normalisés
 * Basé sur:
 *  - 1ère ligne optionnelle: "chapter: A > B > C"
 *  - Types de questions présents: DragMatch exclusivement => suffixe DragMatch
 *  - Version conservée si _v\d+ présent; sinon pas de suffixe version
 * Usage:
 *   npm run rename:courses           # Dry-run: affiche les propositions
 *   npm run rename:courses -- --apply  # Applique réellement les renommages
 */
...
function walkTxt(dir: string, out: string[] = []) {
...
function detectTypes(content: string): { onlyDragMatch: boolean; hasAny: boolean } {
...
function proposeName(absPath: string): { to: string; reason: string } | null {
  ...
  if (path.basename(absPath) === toFile) return null; // déjà OK
...
function main() {
  ...
  const plan: Array<{ from: string; to: string; reason: string }> = [];
  ...
  // Appliquer en veillant à créer les dossiers si nécessaire
  for (const p of plan) {
