...
function setupEventListeners() {
    ...
    if (selectAllBtn) selectAllBtn.addEventListener('click', selectAllThemes);
    if (clearBtn) clearBtn.addEventListener('click', clearAllThemes);
...
function renderThemes() {
    ...
    availableQuestions.forEach(q => {
        if (q.topics) q.topics.forEach(t => allThemes.add(t));
    });
...
function selectAllThemes() {
    ...
    availableQuestions.forEach(q => {
        if (q.topics) q.topics.forEach(t => selectedThemes.add(t));
    });
    updateThemeUI();
    updateSummary();
}
...
function updateSummary() {
    const questions = availableQuestions.filter(q =>
        q.topics?.some(t => selectedThemes.has(t))
    );
