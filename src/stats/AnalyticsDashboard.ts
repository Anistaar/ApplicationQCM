...
import { analyticsFunctions, type RetentionDataPoint, type ProblemQuestion, type WeakZone } from './AnalyticsFunctions';
...
export class AnalyticsDashboard {
  ...
  private async renderRetentionChart(): Promise<void> {
    ...
    section.querySelectorAll('.chart-tab').forEach(tab => {
      tab.addEventListener('click', async (e) => {
        const days = parseInt((e.target as HTMLElement).dataset.days || '7') as 7 | 14 | 30;
   ...
   */
  private async renderRetentionChartData(days: 7 | 14 | 30): Promise<void> {
    const data = await analyticsFunctions.computeRetentionCurve(days);
    
    const canvas = document.getElementById('retention-chart') as HTMLCanvasElement;
    if (!canvas) return;
   ...
   */
  private async renderProblemQuestionsChart(): Promise<void> {
    ...
    const canvas = document.getElementById('problems-chart') as HTMLCanvasElement;
    if (!canvas) return;
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: top10.map(p => {
            if (p.failRate >= 0.75) return '#ef4444';
            if (p.failRate >= 0.6) return '#f59e0b';
            return '#eab308';
          }),
          borderRadius: 4,
        }]
   ...
   */
  private async renderWeakZonesChart(): Promise<void> {
    ...
    const canvas = document.getElementById('weak-zones-chart') as HTMLCanvasElement;
    if (!canvas) return;
   ...
   */
  private async renderVelocitySection(): Promise<void> {
    const velocity = await analyticsFunctions.computeVelocity();
   ...
   */
  private async renderStreakCalendar(): Promise<void> {
    const streak = await analyticsFunctions.computeStreak();
   ...
   * Export section
   */
  private async renderExportSection(): Promise<void> {
    const section = document.createElement('div');
  ...
  private getStreakEncouragement(streak: number): string {
    if (streak >= 30) return '🏆 Incroyable ! 30+ jours consécutifs !';
    if (streak >= 14) return '🔥 Excellente série ! Continuez !';
    if (streak >= 7) return '💪 Super ! Une semaine de suite !';
    if (streak >= 3) return '👍 Bon départ ! Continuez !';
