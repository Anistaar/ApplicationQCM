...
import { questionDB, type QuestionRecord, type ThemeDefinition, type SubjectDefinition } from '../database/QuestionDatabase';
import { importText2QuizFile, analyzeText2QuizFile, type ImportResult } from '../database/ImportService';
...
async function init() {
  try {
    ...
    showToast('Erreur d\'initialisation: ' + (error as Error).message, 'error');
  }
}
...
async function loadDashboardStats() {
  try {
    ...
    showToast('Erreur de chargement des stats: ' + (error as Error).message, 'error');
  }
}
...
async function handleImport() {
  ...
  const subject = (document.getElementById('import-subject') as HTMLSelectElement).value;
  if (!subject) {
    showToast('Veuillez sélectionner une matière', 'warning');
    return;
  }
  
  const defaultThemesInput = (document.getElementById('import-default-themes') as HTMLInputElement).value;
  const defaultThemes = defaultThemesInput ? defaultThemesInput.split(',').map(t => t.trim()) : [];
  const autoDetect = (document.getElementById('import-auto-detect') as HTMLInputElement).checked;
  const overwrite = (document.getElementById('import-overwrite') as HTMLInputElement).checked;
  ...
  for (const file of selectedFiles) {
    try {
      ...
      logPre.textContent += `❌ Erreur: ${(error as Error).message}\n`;
      totalErrors++;
    }
  }
...
async function loadQuestions() {
  ...
  try {
    ...
    const subjectFilter = document.getElementById('filter-subject') as HTMLSelectElement;
    ...
    showToast('Erreur de chargement: ' + (error as Error).message, 'error');
  } finally {
    loading.classList.remove('active');
  }
}

function renderQuestions(questions: QuestionRecord[]) {
  const tbody = document.getElementById('questions-tbody')!;
  tbody.innerHTML = '';
...
async function filterQuestions() {
  const searchTerm = (document.getElementById('search-questions') as HTMLInputElement).value.toLowerCase();
  const subjectFilter = (document.getElementById('filter-subject') as HTMLSelectElement).value;
  const difficultyFilter = (document.getElementById('filter-difficulty') as HTMLSelectElement).value;
  
  let questions = await questionDB.getAllQuestions();
  
  if (searchTerm) {
    questions = questions.filter(q => 
      q.question.question.toLowerCase().includes(searchTerm) ||
...
async function exportDatabase() {
  try {
    ...
    showToast('Erreur d\'export: ' + (error as Error).message, 'error');
  }
}
...
async function initializeDefaultData() {
  ...
  if (subjects.length === 0) {
    ...
    const defaultSubjects: SubjectDefinition[] = [
...
(window as any).editQuestion = async (id: string) => {
  showToast('Fonctionnalité en développement', 'info');
};

(window as any).deleteQuestion = async (id: string) => {
