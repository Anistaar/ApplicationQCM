...
export class QuestionDatabase {
    ...
    async getQuestionsByTheme(theme) {
        if (!this.db)
            throw new Error('Database not initialized');
        return new Promise((resolve, reject) => {
            const tx = this.db.transaction(['questions'], 'readonly');
    ...
    async getQuestionsByThemes(themes) {
        ...
        return allQuestions.filter(q => themes.some(theme => q.themes.includes(theme)));
    }
    async searchQuestions(query) {
        ...
        return allQuestions.filter(q => q.question.question.toLowerCase().includes(lowerQuery) ||
    ...
    async getThemesBySubject(subject) {
        if (!this.db)
            throw new Error('Database not initialized');
