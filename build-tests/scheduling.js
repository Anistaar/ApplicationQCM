...
export function requiredFromSeverityBucket(b) {
    if (b === 'mild')
        return 1;
    if (b === 'medium')
        return 2;
...
export function bucketizeSeverity(s) {
    if (s <= 0.25)
        return 'mild';
    if (s <= 0.6)
        return 'medium';
...
function isValueEqual(val, q) {
    if (!val)
        return false;
    if (q.type === 'QR' && q.answers)
...
export function updateStatAfterAnswer(q, correct, severity, timeMs) {
    ...
    const cur = stats[id] || { box: 1, streak: 0, last: 0, next: 0, required: 1, lastSeverity: undefined, seen: 0, correct: 0, strength: 0, avgTimeMs: 0 };
