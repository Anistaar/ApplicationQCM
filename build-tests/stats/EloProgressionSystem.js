...
class EloProgressionSystem {
    constructor() {
        this.STORAGE_KEY = 't2q_elo_progress_v1';
    }
    /**
     ...
     */
    async updateAfterAnswer(q, correct, timeMs) {
     ...
     */
    computeEloChange(playerElo, opponentElo, won, bonus = 0) {
     ...
     */
    computeTimeBonus(timeMs, correct) {
        if (!correct)
            return 0; // Pas de bonus si faux
     ...
     */
    async getWeakestThemes(limit = 3) {
        const progress = await this.loadProgress();
     ...
     */
    async getStrongestThemes(limit = 3) {
        const progress = await this.loadProgress();
        return Object.values(progress.themes)
     ...
     */
    async selectMatchedQuestion(availableQuestions, targetTheme) {
        const progress = await this.loadProgress();
     ...
     */
    async predictSuccessRate(theme) {
        const progress = await this.loadProgress();
        const themeData = progress.themes[theme];
        if (!themeData || themeData.matches < 3)
