 ...
 */
import { bucketizeSeverity, requiredFromSeverityBucket } from '../scheduling';
...
class StatsManager {
    constructor() {
        this.useIndexedDB = true; // Auto-disable on error
    }
    /**
    ...
    async loadStats() {
        if (this.useIndexedDB) {
            try {
                return await idbAdapter.getAllStats();
            }
            catch (error) {
    ...
    async getStat(qId) {
        if (this.useIndexedDB) {
            try {
                return await idbAdapter.getStat(qId);
            }
            catch (error) {
     ...
     */
    async updateStatAfterAnswer(q, correct, severity, timeMs) {
        ...
        if (!correct)
            cur.correct = Math.max(0, (cur.correct || 0) - 0.5);
        // Update average time
     ...
     */
    async getDueQuestions(allQuestions) {
        const stats = await this.loadStats();
        const now = Date.now();
