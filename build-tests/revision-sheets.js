...
export function loadProgressForSheet(sheet) {
    const all = loadAllProgress();
    const existing = all[sheet.id];
    if (existing && existing.version === 1 && existing.sheetId === sheet.id && existing.placed)
        return existing;
    return makeEmptyProgress(sheet);
}
export function saveProgressForSheet(progress) {
    const all = loadAllProgress();
    all[progress.sheetId] = progress;
    saveAllProgress(all);
}
export function resetProgressForSheet(sheet) {
    const all = loadAllProgress();
    delete all[sheet.id];
    saveAllProgress(all);
}
